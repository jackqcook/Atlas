...
final class GovernanceService {
    ...
    func fetchProposals(for groupID: UUID) async throws -> [Proposal] {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.fetchProposals(groupID: groupID)
        }
    ...
    func createProposal(
        ...
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.createProposal(
                title: title,
                body: body,
                type: type,
                groupID: groupID,
    ...
    func castVote(proposalID: UUID, voterID: UUID, choice: VoteChoice) async throws {
        if await DemoStore.shared.isEnabled {
            await DemoStore.shared.castVote(proposalID: proposalID, voterID: voterID, choice: choice)
            return
        }
    ...
    func myVote(proposalID: UUID, voterID: UUID) async throws -> Vote? {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.myVote(proposalID: proposalID, voterID: voterID)
        }
