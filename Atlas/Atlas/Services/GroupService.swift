...
final class GroupService {
    ...
    func fetchMyGroups(userID: UUID) async throws -> [Group] {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.fetchMyGroups(userID: userID)
        }
    ...
    func joinGroup(inviteCode: String, userID: UUID) async throws -> Group {
        if await DemoStore.shared.isEnabled {
            return try await DemoStore.shared.joinGroup(inviteCode: inviteCode, userID: userID)
        }
    ...
    func fetchChannels(for groupID: UUID) async throws -> [Channel] {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.fetchChannels(groupID: groupID)
        }
    ...
    func createChannel(groupID: UUID, name: String, type: ChannelType) async throws -> Channel {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.createChannel(groupID: groupID, name: name, type: type)
        }

        let channel = Channel(
            id: UUID(),
            groupID: groupID,
            name: sanitizedChannelName(name),
            type: type,
            createdAt: Date()
        )
        try await supabase.from("channels").insert(channel).execute()
        return channel
    }

    func fetchMembers(for groupID: UUID) async throws -> [(Membership, User)] {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.fetchMembers(groupID: groupID)
        }
    ...
    func myMembership(userID: UUID, groupID: UUID) async throws -> Membership? {
        if await DemoStore.shared.isEnabled {
            return await DemoStore.shared.myMembership(userID: userID, groupID: groupID)
        }
    ...
    private func sanitizedChannelName(_ name: String) -> String {
        let lowered = name
